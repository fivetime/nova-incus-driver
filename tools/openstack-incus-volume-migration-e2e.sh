...
wait_device_size() {
    ...
    while ((SECONDS < deadline)); do
        current=$(remote "$host" \
            "incus --project '$INCUS_PROJECT' exec '$instance_name' -- blockdev --getsize64 '$DEVICE'" \
            2>/dev/null || true)
...
remote "$SOURCE_SSH" \
    "incus --project '$INCUS_PROJECT' profile device show '$instance_name' | grep -F '$volume_id:'"
remote "$SOURCE_SSH" \
    "incus --project '$INCUS_PROJECT' profile get '$instance_name' \
...
remote "$SOURCE_SSH" "incus --project '$INCUS_PROJECT' exec '$instance_name' -- sh -c \
...
restored_marker=$(remote "$DEST_SSH" \
    "incus --project '$INCUS_PROJECT' exec '$instance_name' -- sh -c \
     'fuse2fs $DEVICE /mnt/cinder >/dev/null 2>&1; cat /mnt/cinder/marker; \
      umount /mnt/cinder'")
