...
while IFS= read -r name; do
    if [[ "$(incus config get "$name" user.openstack.uuid \
...
guest_iface=$(incus config get "$instance_name" "volatile.$iface.name")
[[ -n "$guest_iface" ]]
incus exec "$instance_name" -- ip link set "$guest_iface" up
incus exec "$instance_name" -- sh -c 'command -v dhcpcd' >/dev/null
incus exec "$instance_name" -- dhcpcd -x "$guest_iface" >/dev/null 2>&1 || true
incus exec "$instance_name" -- dhcpcd -4 -L -1 -t "$TIMEOUT" "$guest_iface"
...
incus exec "$instance_name" -- ip -4 -o addr show dev "$guest_iface" |
    grep -Fq " $fixed_ip/"
...
! incus config show "$instance_name" --expanded | grep -q "$iface"
