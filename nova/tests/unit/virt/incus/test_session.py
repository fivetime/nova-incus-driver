...
class SessionContainerTest(test.NoDBTestCase):
    ...
    def setUp(self):
        ...
        self.session = session.IncusAPISession()
    ...
    @stubs.annotated_data(
        ('api_fail', incus_exceptions.APIError(500, 'Fake'),
         exception.NovaException),
    )
    def test_container_init_fail(self, tag, side_effect, expected):
        """
        continer_init create as container on a given Incus host. Make
        sure that we reaise an exception.NovaException if there is
        an APIError from the Incus API.
        """
        config = mock.Mock()
        instance = stubs._fake_instance()
