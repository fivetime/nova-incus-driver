...
class TestAttachEphemeral(test.NoDBTestCase):
    """Tests for nova.virt.incus.storage.attach_ephemeral."""
    ...
    def setUp(self):
        ...
        CONF_patcher = mock.patch('nova.virt.incus.common.conf.CONF')
    ...
    @mock.patch(
        'nova.virt.incus.storage.driver.block_device_info_get_ephemerals')
    def test_add_ephemerals_with_zfs(
        ...
        storage.attach_ephemeral(
            client, block_device_info, incus_config, instance)
    ...
    def test_add_ephemerals_with_btrfs(
        ...
        block_device_info = mock.Mock()
        incus_config = {'environment': {'storage': 'btrfs'}}
        profile = mock.Mock()
        profile.devices = {
            'root': {
                'path': '/',
...
class TestDetachEphemeral(test.NoDBTestCase):
    """Tests for nova.virt.incus.storage.detach_ephemeral."""
    ...
    def test_remove_ephemeral_with_lvm(
        ...
        expected_calls = [
            mock.call(
                'umount', '/dev/incus/instance-00000001-ephemerals0',
                run_as_root=True),
