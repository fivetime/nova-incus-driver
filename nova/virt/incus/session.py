...
class IncusAPISession(object):
    """The session to invoke the Incus API session."""
    ...
    def get_session(self, host=None):
        ...
        try:
            ...
            # via an rpc call
            LOG.exception("Connection to Incus failed")
            payload = dict(ip=CONF.host,
                           method='_connect',
                           reason=ex)
            rpc.get_notifier('compute').error(nova_context.get_admin_context,
                                              'compute.nova_incus.error',
                                              payload)
    ...
    def container_init(self, config, instance, host=None):
        """Create a Incus container

        :param config: Incus container config as a dict
    ...
    def operation_wait(self, operation_id, instance, host=None):
        ...
        try:
            ...
            msg = _("Failed to communicate with Incus API {instance}: "
            ...
            with excutils.save_and_reraise_exception():
                LOG.error("Error from Incus during operation wait "
                          "{instance}: {reason}"
    ...
    def operation_info(self, operation_id, instance, host=None):
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.error("Error from Incus during operation_info "
                          "{instance}: {reason}"
                          .format(instance=instance.image_ref, reason=e))
    ...
    def container_migrate(self, instance_name, host, instance):
        """Initialize a container migration for Incus
